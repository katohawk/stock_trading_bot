...
class BacktestEngine:
    ...
    def run(
        ...
        for i in range(len(data)):
            ...
            for sig in self.risk.check_stop_loss(account, current_prices):
                if sig.symbol in positions:
                    ...
                    cash += pos.quantity * fill_price * (1 - self.config.commission_rate)
                    daily_pnl += pos.quantity * (fill_price - pos.avg_cost)
                    self.trades.append({"datetime": dt, "symbol": sig.symbol, "side": "sell", "qty": pos.quantity, "price": fill_price, "reason": sig.reason})
                    daily_trade_count += 1
                ...
                if signal.direction == SignalDirection.LONG:
                    ...
                    if delta_value > 1e-6:  # 加仓
                        ...
                        if cost > 0:
                            ...
                            daily_trade_count += 1
                            self.trades.append({"datetime": dt, "symbol": symbol, "side": "buy", "qty": qty, "price": fill_price, "reason": signal.reason})
                        ...
                        if sell_qty > 0:
                            fill_price = price * (1 - self.config.slippage_pct)
                            cash += sell_qty * fill_price * (1 - self.config.commission_rate)
                            daily_pnl += sell_qty * (fill_price - pos.avg_cost)
                            ...
                            daily_trade_count += 1
                            self.trades.append({"datetime": dt, "symbol": symbol, "side": "sell", "qty": sell_qty, "price": fill_price, "reason": signal.reason})
                elif signal.direction == SignalDirection.FLAT and symbol in positions:
                    pos = positions.pop(symbol)
                    fill_price = price * (1 - self.config.slippage_pct)
                    cash += pos.quantity * fill_price * (1 - self.config.commission_rate)
                    daily_pnl += pos.quantity * (fill_price - pos.avg_cost)
                    self.trades.append({"datetime": dt, "symbol": symbol, "side": "sell", "qty": pos.quantity, "price": fill_price, "reason": signal.reason})
                    daily_trade_count += 1
            ...
            # 新的一天重置日统计（放在使用 daily_pnl 之后，下一日开头更新）
        ...
        for sym, pos in list(positions.items()):
            ...
            cash += pos.quantity * price * (1 - self.config.commission_rate)
            self.trades.append({"datetime": data.index[-1], "symbol": sym, "side": "sell", "qty": pos.quantity, "price": price, "reason": "close"})
        final_equity = cash
